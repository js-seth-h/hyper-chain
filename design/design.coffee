...
describe '?', ()->
  it '?', (done)->
    ###
      ...
      SIMD면 충분하고, 타이밍 제어는 Boxlet 수행할수 있다.
      다만. Boxlet가 끝났는지를 어떻게 알 것인가?  이건 Boxlet가 알아서...


      Boxlet, 데이터를 밀어 넣는 장치
      ...
      의문1. Boxlet는 callback을 수신 한다?안한다?
        ...
        Boxlet = chain with self trigger가 되어 버려서 의미없는 논의
        chain이 다른 체인(=Boxlet)에 편입될지 말지는 concatTo, forkFrom 구분하여 3rd의 판단에 맞김
        > 이거 틀렸음. Boxlet는 모듈이고, Timing 제어권이 있어서 Boxlet가 Callbak수신여부를 판단해야함.


      event 'end', Enfocer가 끝을 인식 했을떄, 무한 스트림은 안생긴다. ex> EventStream
      Boxlet는 능동체라서 'drain'은 없을듯하다.

      체인은  WritableStream과 유사.
        > 그러나 체인은 데이터를 보관하지 않는다. 흘려 보낸다.
      Boxlet는 ReaderbleStream과 유사하다.
      ...
      hc.hook.ser는 어떻게 되냐?
        ...
        아래 처럼 되어서, Callback여부가 Boxlet 책임하에 가야함.
          reactTo = (Boxlet)->
          ...
          타이밍 제어를 위해서는 Boxlet가 알아함.

      Boxlet is a EventEmitter 'end'
      Boxlet is not Function - 실행불가. 실행시 해야할 작업 불명
      Boxlet is a Object

      Boxlet has call Reactors, 이를 위한 판단 로직, Function call의 진입점(~트리거)을 가짐.
      ...
      Boxlet는
        최소한 Timing(함수 실행)을 가져야한다.
          즉 hook.event 처럼 최소한것 것만 가져도 Boxlet다.
    ...
    chain = hc() # return function
      ...
      .reactTo hc.hook.of hc.Boxlet.ser messages... # Input Queue에 넣고 시작함. messages를 하나씩 처리하도록 함 Serial? Parallel? 기본은 직렬, 하나씩 끝내자
      .reactTo hc.hook.of hc.Boxlet.par messages... # 메시지를 동시에 뿌려버린다.
      ...
      hook.of = (opt, target)->
        ...
        return hook =
          off: (a_chain)->
            Boxlet[opt.unsetter] a_chain

          on : (a_chain)->
            Boxlet[opt.setter] a_chain
      ...
      ###
      # .event src, 'event_name' 도 가능하다. 짧고 직접적. 3단어 짧다 reactTo hc.Boxlet
      # 늘 그렇듯이 간접층이 없으면, 여러 문제를 해결하기가 어렵지..
      # 게다가 참 다양한 Boxlet형태가 있을텐데, 다 구현해넣기도 무리. 짧게 쓸방법은..?
